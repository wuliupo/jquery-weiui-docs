      ...
      for (var l = contentSection.length; l--;) {
        if ((topCache[l] - currentTop) < windowHeight) {
          ...
          if (!contentSectionItem.hasClass('informational')) {
			var hash = contentSectionItem.data("url");
            var iframe = $("#window iframe").get(0);
            if(iframe && hash) {
              hash = '#' + hash;
              if(iframe.src.indexOf(hash) === -1) {
                iframe.contentWindow.document.location.hash = hash;
                coder.makeCode(iframe.contentWindow.document.location.href);
				$('#preview').attr('href', iframe.contentWindow.document.location.href);
              }
            }
          }
          break;
